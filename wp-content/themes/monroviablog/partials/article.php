...
<article id="post-<?php the_ID(); ?>" <?php post_class('media well'); ?>>
    ...
    <div class="header-container">
        ...
        if (has_post_thumbnail() && !is_single() && !is_page() && $monroviablog_theme_options['article_featured_images'] != 'none') : ?>
            <a class="media-object pull-<?php echo $monroviablog_theme_options['article_featured_images']; ?>" href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
        ...
        <header class="media-body">
            ...
            <?php if (!is_page()) : ?>
                <div class="media-heading text-muted">
                    ...
                    // Comment number
                    if (comments_open()) :
                        $output .= '&nbsp;&nbsp;<strong><i class="icon-comments"></i></strong>&nbsp;<a href="'.get_permalink().'#comments" title="'.__('Comments','monroviablog').'">';
                        $ncom=get_comments_number(); if ($ncom==0) $output .= __('no comments', 'monroviablog'); elseif ($ncom==1) $output .= __('1 comment', 'monroviablog'); else $output .= sprintf (__('%s comments','monroviablog'), $ncom);
                        $output .= '</a>';
                    endif;
                    ...
                    edit_post_link( __('Edit', 'monroviablog'), '&nbsp;&nbsp;<i class="icon-pencil"></i>&nbsp;<span class="edit-link">', '</span>');
                    ?>
                </div>
            <?php endif; ?>
        </header>
    ...
    <section>
        ...
        if (is_single() OR is_page()) :
            ...
            wp_link_pages(array(
                ...
                'nextpagelink' => __('Next page', 'monroviablog'),
                'previouspagelink' => __('Previous page', 'monroviablog'),
                'pagelink' => '%',
                'link_before' => '<span class="ft-btn">',
                'link_after' => '</span>',
                'before' => '<div class="clearfix"></div><br />' . __('Pages:', 'monroviablog') . ' <div class="ft-article-pages">',
                'after' => '</div>'
            ));
