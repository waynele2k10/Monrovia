...
function change_social_network_type(obj)
{
	...
	var children = jQuery(obj).parents('div:not(.select-wrap)').children();
...
function writeNewFileLabel(id, value){	
	...
	jQuery(id).value = splitArray[indexArray];	
}
...
jQuery(document).ready( function($){
	...
	var form = jQuery('create_profile_form')||jQuery('edit_profile_form');
	if(form){
		form.custom_validation = function(){			
			var ret = true;
			if ( jQuery('[name="new_password_reenter"]').length ){
				var errorMsgPass = document.getElementById('errors-pass');
				if(jQuery('[name="new_password"]')[0].value !== jQuery('[name="new_password_reenter"]')[0].value){
					jQuery('[name="new_password_reenter"]')[0].addClassName('error_validation');				
					errorMsgPass.innerHTML = 'Your password does not match.';
					errorMsgPass.style.display = "block";
					ret = false;
				}else{
					jQuery('[name="new_password_reenter"]')[0].removeClassName('error_validation');
					errorMsgPass.style.display = "none";
				}
			}
			if(!jQuery('[name="expertise[]"]:checked').length){
				jQuery('.expertise_checkboxes')[0].addClassName('error_validation');
				ret = false;
			}else{
				jQuery('.expertise_checkboxes')[0].removeClassName('error_validation');
			}
			if(!jQuery('[name="services[]"]:checked').length){
				jQuery('.services_checkboxes')[0].addClassName('error_validation');
				ret = false;
			}else{
				jQuery('.services_checkboxes')[0].removeClassName('error_validation');
			}
