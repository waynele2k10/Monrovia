...
function IContactSubscribe($account_id, $key, $user, $pass, $email, $list_id, &$result_str)
{
  ...
  $ch=curl_init("https://app.icontact.com/icp/a/$account_id/c/18663/contacts/?email=".URLEncode($email));
  curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
  curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
  ...
  // Add subscription
  $ch=curl_init("https://app.icontact.com/icp/a/$account_id/c/18663/subscriptions/");
  curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  ...
  // Set result string
  $result_str = ($result ? "You have been successfully signed up!" : $buf);

  // Return result
  return $result;
}

function IContactAddContact($account_id, $key, $user, $pass, $email, $firstname, $lastname, $zipcode, $coldzone,
&$result_str)
{
  // Get client folder id
  if (!IContactLogin($account_id, $key, $user, $pass, $client_folder_id))
  ...
  $data.= "<postalCode>$zipcode</postalCode>\r\n";
  $data.= "<coldzone>$coldzone</coldzone>\r\n";
  $data.= "<status>normal</status>\r\n";
  $data.= "</contact>\r\n</contacts>";

  // Add contact
  $ch=curl_init("https://app.icontact.com/icp/a/$account_id/c/18663/contacts/");
  curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
  curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
  curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
  curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
