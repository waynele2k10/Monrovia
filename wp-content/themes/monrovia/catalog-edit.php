...
<?php } ?>
    ...
    <div class="content_wrapper clear">
        ...
        <?php the_breadcrumb($post->ID); //Print the Breadcrumb ?>
            ...
            <?php endwhile; endif; ?>
            <!-- Code Imported from OLD monrovia.com -->
    <?php        
    $catalog_limit = 500;
	...
	function before_output(){
		...
		global $catalog_id, $catalog, $userID;
		$catalog_id = intval($_GET['id']);

		if($catalog_id==0){
			if($_GET['id']=='new'){
				$catalog = new catalog();
				$catalog->info['user_id'] = $userID;
				if(isset($_GET['original_id'])){
					...
					$original_catalog_belongs_to_user = ($original_catalog->info['user_id']==$userID);
					...
					if($catalog->info['monrovia_locations']=='') $catalog->info['monrovia_locations'] = 'azusa,visalia,dayton,cairo,granby';
			...
			}else{ // End if Original =  new
				header('location:../catalogs/');
				exit;				
			}
		}else{  //End if catalog = 0
			$catalog = new catalog($catalog_id);
			$catalog->generate_covers(true);
			if($catalog->info['user_id']!=$userID){
				header('location:/catalogs/');
				exit;
			}
		}
		...
		if(isset($_POST['save'])&&$_POST['save']=='1'){
			echo $name = trim($_POST['catalog']['name']);
			$plant_ids = trim(trim($_POST['catalog']['plant_ids'],','));
			
			$catalog->get_plant_count();
			if(intval($catalog->info['plant_count'])>500){
				header('location:/catalogs/?msg=error_not_saved');
				exit;
			}else{
				if($name!=''){
					$catalog->info['name'] = escapeApostrophes($name);
					$catalog->info['plant_ids'] = $plant_ids;

					// CATALOG COVER FIELDS
					if(isset($catalog->info['template_id'])&&intval($catalog->info['template_id'])>0){
						$catalog->info['title'] = escapeApostrophes(trim($_POST['catalog']['title']));
						$catalog->info['customer_contact'] = escapeApostrophes(trim($_POST['catalog']['customer_contact']));
						$catalog->info['customer'] = escapeApostrophes(trim($_POST['catalog']['customer']));
						$catalog->info['sales_rep_name'] = escapeApostrophes(trim($_POST['catalog']['sales_rep_name']));
						$catalog->info['additional_info_1'] = escapeApostrophes(trim($_POST['catalog']['additional_info_1']));
						$catalog->info['additional_info_2'] = escapeApostrophes(trim($_POST['catalog']['additional_info_2']));
						$catalog->info['additional_info_3'] = escapeApostrophes(trim($_POST['catalog']['additional_info_3']));
	...
	function output_modals(){
		global $catalog, $plant_images, $current_user, $upload_dir;
		global $current_user;
		get_currentuserinfo();
	...
	<style>
	    ...
	    .plant_image_field {
            background: url('../../wp-content/uploads/loading.gif') center 45px no-repeat; 
        }
                        ...
                        <?php $upload_dir = wp_upload_dir(); ?>
							...
							<div style="width:50%;float:left;">
								<div class="padding" style="padding-right:16px;">
									...
									<br />
									<img src="<?php echo $upload_dir['url'];?>/catalog_directions_edit_name.gif" style="margin-top:.5em;width:182px;height:81px;" />
								</div>
							</div>
							<div style="width:50%;float:left">
								<div class="padding">
									...
									<br />
									<img src="<?php echo $upload_dir['url'];?>/catalog_directions_edit_search.gif" style="margin-top:.5em;width:142px;height:106px;" />
								</div>
							</div>
							<div style="clear:both;"></div>
							<div style="float:left;">
								<div class="padding">
									...
									<br />
									<img src="<?php echo $upload_dir['url'];?>/catalog_directions_edit_select.gif" style="margin-top:.5em;width:111px;height:74px;" />
								</div>
							</div>
							<div style="clear:both;"></div>
							<div style="float:left">
								<div class="padding">
									...
									<br />
									<img src="<?php echo $upload_dir['url'];?>/catalog_directions_edit_generate.gif" style="margin-top:.5em;width:240px;height:123px;" />
								</div>
							</div>
							<div style="clear:both;"></div>
							<div style="padding-top:12px;bottom:0px;position:relative;text-align:center;">
		...
		<?php if(isset($catalog->info['template_id'])&&intval($catalog->info['template_id'])>0){ ?>
				...
				<table class="modal_dialog_backing">
					...
					<tr>
						...
						<td class="corner_middle">
								...
								<div class="slimTabBlurb sel" tab="1">
									<div class="field_groups">
										<div class="field_group">
											<label>Title</label>
											<input class="text_field" maxlength="40" name="cover[title]" mapped_field="title" value="<?php echo (isset($catalog->info['title'])?html_sanitize($catalog->info['title']):'')?>" />
											<div class="example">
												Example: New Plants for <?php echo date('Y');?>
											</div>
										</div>
									</div>
									<div class="cover_images">
										<div class="cover_image">
											<img src="<?php echo $catalog->info['cover_image_paths']['front']?>?rnd=<?php echo time()?>" class="cover_thumbnail_front" />
											<div class="outline" style="left:13px;top:53px;width:125px;height:5px;" mapped_field="title"></div>
										</div>
										<label>Front cover</label>
										<div class="cover_image">
											<img src="<?php echo $catalog->info['cover_image_paths']['back']?>?rnd=<?php echo time()?>" class="cover_thumbnail_back" />
								...
								<div class="slimTabBlurb" tab="2">
									<div class="field_groups">
										<div class="field_group">
											...
											<input class="text_field" maxlength="40" name="cover[customer]" mapped_field="customer" value="<?php echo (isset($catalog->info['customer'])?html_sanitize($catalog->info['customer']):'')?>" />
											<div class="example">
										...
										<div class="field_group">
											...
											<input class="text_field" maxlength="40" name="cover[customer_contact]" mapped_field="customer_contact" value="<?php echo (isset($catalog->info['customer_contact'])?html_sanitize($catalog->info['customer_contact']):'')?>" />
											<div class="example">
												Example: John Smith
											</div>
										</div>
									...
									<div class="plant_image_field">
                                        <?php if(isset($catalog->info['plant_1_image_set_id'])&&intval($catalog->info['plant_1_image_set_id'])>0){ ?>
                                             <img src="<?php echo $upload_dir['url'];?>/plants/image_set_thumbail.php?id=<?php echo $catalog->info['plant_1_image_set_id']?>&width=90&height=90" class="image_thumbnail" />
                                        <?php }else{ ?>
                                             <img src="<?php echo $upload_dir['url'];?>/catalog_cover_plant_random.gif" class="image_thumbnail" />
                                        <?php } ?>
                                        <div class="fields">
                                             <label>Plant image (left)</label>
                                             <select name="cover[plant_1_id]" mapped_field="plant_1" _value="<?php echo $plant_images[0]['plant_id']?>" onChange="plant_dropdown_changed('1',window.event||event);">
                                                   <option value="">(Random plant)</option>
                                              </select>
                                              <label>Plant image</label>
                                               <select name="cover[plant_1_image_set_id]" mapped_field="plant_1" _value="<?php echo $plant_images[0]['image_set_id']?>" onChange="plant_image_dropdown_changed('1',window.event||event);"></select>
                                        </div>
                                     </div>

                                     <div class="plant_image_field">
                                          <?php if(isset($catalog->info['plant_2_image_set_id'])&&intval($catalog->info['plant_2_image_set_id'])>0){ ?>
                                              <img src="<?php echo $upload_dir['url'];?>/plants/image_set_thumbail.php?id=<?php echo $catalog->info['plant_2_image_set_id']?>&width=90&height=90" class="image_thumbnail" />
                                          <?php }else{ ?>
                                              <img src="<?php echo $upload_dir['url'];?>/catalog_cover_plant_random.gif" class="image_thumbnail" />
                                          <?php } ?>
                                          <div class="fields">
                                               <label>Plant image (center)</label>
                                               <select name="cover[plant_2_id]" mapped_field="plant_2" _value="<?php echo $plant_images[1]['plant_id']?>" onChange="plant_dropdown_changed('2',window.event||event);">
                                                    <option value="">(Random plant)</option>
                                               </select>
                                               <label>Plant image</label>
                                               <select name="cover[plant_2_image_set_id]" mapped_field="plant_2" _value="<?php echo $plant_images[1]['image_set_id']?>" onChange="plant_image_dropdown_changed('2',window.event||event);"></select>
                                           </div>
                                      </div>

                                      <div class="plant_image_field">
                                           <?php if(isset($catalog->info['plant_3_image_set_id'])&&intval($catalog->info['plant_3_image_set_id'])>0){ ?>
                                                <img src="<?php echo $upload_dir['url'];?>/plants/image_set_thumbail.php?id=<?php echo $catalog->info['plant_3_image_set_id']?>&width=90&height=90" class="image_thumbnail" />
                                           <?php }else{ ?>
                                                <img src="<?php echo $upload_dir['url'];?>/catalog_cover_plant_random.gif" class="image_thumbnail" />
                                           <?php } ?>
                                           <div class="fields">
                                               <label>Plant image (right)</label>
                                               <select name="cover[plant_3_id]" mapped_field="plant_3" _value="<?php echo $plant_images[2]['plant_id']?>" onChange="plant_dropdown_changed('3',window.event||event);">
                                                    <option value="">(Random plant)</option>
                                               </select>
                                               <label>Plant image</label>
                                               <select name="cover[plant_3_image_set_id]" mapped_field="plant_3" _value="<?php echo $plant_images[2]['image_set_id']?>" onChange="plant_image_dropdown_changed('3',window.event||event);"></select>
                                           </div>
                                       </div>
                                       <div style="clear:both;"></div>
								</div>
								<div class="slimTabBlurb" tab="3">
									<div class="field_groups">
										<div class="field_group">
											...
											<input class="text_field" maxlength="40" name="cover[sales_rep_name]" mapped_field="sales_rep_name" value="<?php echo (isset($catalog->info['sales_rep_name'])?html_sanitize($catalog->info['sales_rep_name']):'')?>" />
											<div class="example">
												<?
												$example = 'John Smith';
												if($current_user->user_firstname) $example = $current_user->user_firstname.' '.$current_user->user_lastname;
												?>
										...
										<div class="field_group halfsize left">
											...
											<input class="text_field" maxlength="40" name="cover[additional_info_1]" mapped_field="additional_info_1" value="<?php echo (isset($catalog->info['additional_info_1'])?html_sanitize($catalog->info['additional_info_1']):'')?>" />
											<div class="example">
										...
										<div class="field_group halfsize left">
											...
											<input class="text_field" maxlength="40" name="cover[additional_info_2]" mapped_field="additional_info_2" value="<?php echo (isset($catalog->info['additional_info_2'])?html_sanitize($catalog->info['additional_info_2']):'')?>" />
											<div class="example">
												Examples: Address, Phone, Fax, Website
											</div>
										...
										<div class="field_group">
											...
											<input class="text_field" maxlength="40" name="cover[additional_info_3]" mapped_field="additional_info_3" value="<?php echo (isset($catalog->info['additional_info_3'])?html_sanitize($catalog->info['additional_info_3']):'')?>" />
											<div class="example">
												Examples: Address, Phone, Fax, Website
											</div>
										</div>
										...
										<div class="field_group halfsize left">
											<div id="add_location">
												...
												<div class="clear">
													...
													<a href="javascript:void(0);" onClick="add_monrovia_location();" class="green-btn left">add</a>
												</div>
											</div>
							...
							<div style="">
								<div style="padding-top:12px;bottom:0px;position:relative;text-align:center;">
									<a href="javascript:void(0);" class="green-btn left" onClick="modal_hide();show_unobtrusive_message('Your changes will not be saved until you click the &quot;Save Changes&quot; button.',5000);">OK</a>
								</div>
							</div>
						</td>
						<td class="corner corner_right"></td>
...
<script>
	var catalog = {
		'plant_count':<?php echo (isset($catalog->info['plant_count'])?intval($catalog->info['plant_count']):'0')?>,
		'plants':<?php echo $json_results?>,
		'plants_selected_ids':[],
		'template_id':'<?php echo (isset($catalog->info['template_id'])?intval($catalog->info['template_id']):'0')?>',
		'limit':<?php echo ($catalog_limit.'')?>,
		'plant_images':<?php echo to_json($plant_images)?>
	}
</script>
<script src="<?php site_url();?>/js/catalogs_edit.js?rnd=3"></script>
<script src="<?php site_url();?>/js/slim_tabs.js" type="text/javascript"></script>
<link href="<?php site_url();?>/css/catalogs.css?rnd=1" rel="stylesheet" type="text/css" />
<link href="<?php site_url();?>/css/slim_tabs.css" rel="stylesheet" type="text/css" />
<link href="<?php site_url();?>/css/modal.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="<?php site_url();?>/js/jquery.min.js"></script>
<script type="text/javascript">
jQuery.noConflict();
</script>
<script src="/js/modal.js" type="text/javascript"></script>
<script type="text/javascript" src="<?php site_url();?>/js/jquery.form.js"></script>
<script type="text/javascript" src="<?php site_url();?>/js/csv_importer.js"></script>
<div class="create-catalog">		
		<div style="padding-top:1.5em;">
			...
			<div style="width:100%;">
				<div style="padding-right:212px;"><?php output_action_group(); ?></div>
			</div>
			<div id="column_left" class="columns">
				<table class="module_wrapper list" style="margin-right:0px;width:100%;margin-bottom:10px;">
				<tr>
							...
							<form action="" method="get" id="form_plant_search_item_number">
								<div class="black">
									<div class="filter_section">
										...
										<input name="item_number" class="text_field" maxlength="5" type="text" />
									</div>
								</div>
				...
				<tbody><tr>
					...
					<td class="content" style="padding:10px;">
						...
						<div class="list_content" style="padding-bottom:16px;">
								<div class="black">
									<div class="filter_section">
										<form id="csv-import-form" action="http://<?php echo $_SERVER['HTTP_HOST'];?>/pdfcatalogs/inc/csv-file-import.php" method="post" enctype="multipart/form-data">
											<label style="display:inline-block;" for="csv-file">CSV file:</label>
											<input style="width: 75px;font-size: 10px;" type="file" size="60" name="csv-file" class="aaa" id="csv-file"><br>
											<br>
											<div style="text-align:center;">
											...
											<div id="csv-import-form-files"></div>
											<div class="hr"></div>
											<center><a href="/pdfcatalogs/assets/plants.csv" target="_blank" download>Download Template</a></center>
										</form>
									</div>
								</div>								
						</div>
							...
							<form action="" method="get" id="form_plant_search">
								<div id="module_refine_search">
									<div class="filter_section">
										...
										<input name="common_name" class="text_field" type="text" />
									</div>
									<div class="filter_section">
										<label>Botanical name:</label>
										<input name="botanical_name" class="text_field"type="text" />
									</div>
			...
			<div id="results_column">
				...
				<table class="module_wrapper list" id="results_table">
						...
						<?php output_action_group(); ?>
						
						
			</div>

			<div id="right_column" class="columns">
					...
					<tr>
						...
						<td>
							<div>
								...
								<input id="catalog_name" value="<?php echo (isset($catalog->info['name'])?html_sanitize($catalog->info['name']):'')?>" class="text_field" type="text" />
								<?php if(isset($catalog->info['template_id'])&&intval($catalog->info['template_id'])>0){ ?>
									...
									<input id="catalog_title" value="<?php echo (isset($catalog->info['title'])?html_sanitize($catalog->info['title']):'')?>" class="text_field" />
									-->
								<?php } ?>
								<div class="hr"></div>
								<?php if(isset($catalog->info['template_id'])&&intval($catalog->info['template_id'])>0){ ?>
									...
									<div id="covers">
										<div class="item" title="Click to edit" onClick="launch_cover_editor();">
											<img src="<?php echo $catalog->info['cover_image_paths']['front']?>?rnd=<?php echo time()?>" class="thumbnail cover_thumbnail_front" style="width:90px;height:115px;" />
											<div class="label">Front cover</div>
										</div>
										<div class="item" title="Click to edit" onClick="launch_cover_editor();">
											<img src="<?php echo $catalog->info['cover_image_paths']['back']?>?rnd=<?php echo time()?>" class="thumbnail cover_thumbnail_back" style="width:90px;height:115px;" />
								...
								<div style="padding-bottom:.5em;">
									Limit <?php echo $catalog_limit.''?> plants per catalog.
								</div>

								<div style="padding-top:.75em;">
									By default, your plants will be rendered to your catalog in the order below.
									<br /><br />
									<?php //if(!isset($GLOBALS['browser_info']['family'])||$GLOBALS['browser_info']['family']!='ios'){ ?>
								...
								<div class="hr"></div>
									<p class="clear"><a href="#" onClick="catalog.save();" class="green-btn left">save changes</a></p>
									<a href="./" class="white-btn left">do not save changes</a>
							</div>
						</td>
						<td></td>
					</tr>
		...
		<div style="overflow:hidden;height:0px;width:0px;position:absolute;top:-5000px;">
			<table id="sample_result">
				<tr class="result">
					<td colspan="2" class="thumbnail"><div class="padding"><img style="max-width:none;" src="<?php echo $upload_dir['baseurl']; ?>/spacer.gif" width="64px" /></div></td>
...
<form id="form_save" action="?<?php echo $params?>" method="post">
	...
	<input type="hidden" id="save_monrovia_locations" name="catalog[monrovia_locations]" value="<?php echo (isset($catalog->info['monrovia_locations'])?html_sanitize($catalog->info['monrovia_locations']):'')?>" />
...
<div id="preload" style="display:none;"><img src="<?php echo $upload_dir['url']; ?>/icon_loading_yellow.gif" /></div>
