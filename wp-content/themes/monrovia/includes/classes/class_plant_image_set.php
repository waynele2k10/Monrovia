<?php
	...
	class plant_image_set extends record {
		...
		function save($params = NULL){
			...
			if($this->info['is_primary']=='1'){
				...
				if(is_numeric($this->info['plant_id'])){
					mysql_query("UPDATE $this->table_name SET is_primary=0 WHERE plant_id=".$this->info['plant_id']);
				}
			}
			parent::save();
		...
		function generate_custom_thumbnail($width,$height,$horizontal_alignment = null,$vertical_alignment = null){
			...
			try {
				$dest_filename = $_SERVER['DOCUMENT_ROOT'].'/temp/plant_image_set_' . $this->info['id'] . '_' . time() . '_' . rand() . '.jpg';
