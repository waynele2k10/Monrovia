	...
	public function template_redirect() {
		...
		if ( has_action( 'tml_request_' . $this->request_action ) ) {
			...
			switch ( $this->request_action ) {
				...
				case 'rp' :
					// Dirty hack for now
					global $rp_login, $rp_key, $wpdb;

					$rp_cookie = 'wp-resetpass-' . COOKIEHASH;
					if ( isset( $_GET['key'] ) ) {
						$value = sprintf( '%s:%s', wp_unslash( $_GET['login'] ), wp_unslash( $_GET['key'] ) );
						//setcookie( $rp_cookie, $value, 0, '/', COOKIE_DOMAIN, is_ssl(), true );
                                                $check_exist = $wpdb->get_row('select * from reset_pass_cookie where user_login = "'.$_GET['login'].'"');
                                                if (!empty($check_exist)) {
                                                    ...
                                                    $wpdb->insert( 'reset_pass_cookie', array('user_login'=>$_GET['login'],'key'=>$_GET['key']));
                                                }
						...
						exit;
                                        ...
                                        $key = "";
                                        if(isset( $_GET['login'] )){   
                                            $key = $wpdb->get_row('select * from reset_pass_cookie where user_login = "'.$_GET['login'].'"');
                                        }
                                        if(isset($_POST['user_login'])){   
                                            $key = $wpdb->get_row('select * from reset_pass_cookie where user_login = "'.$_POST['user_login'].'"');
                                        }
					//if ( isset( $_COOKIE[ $rp_cookie ] ) && 0 < strpos( $_COOKIE[ $rp_cookie ], ':' ) ) {
   
                                        if ($key != "") {
						//list( $rp_login, $rp_key ) = explode( ':', wp_unslash( $_COOKIE[ $rp_cookie ] ), 2 );
                                                $rp_login = $key->user_login;
                                                $rp_key = $key->key;
					...
					if ( ! $user || is_wp_error( $user ) ) {
						//setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, '/', COOKIE_DOMAIN, is_ssl(), true );
                                                if(isset($_GET['login']))
                                                    $wpdb->delete( 'reset_pass_cookie', array('user_login'=>$_GET['login']), array('%s') );
                                                if(isset($_POST['user_login']))
                                                    $wpdb->delete( 'reset_pass_cookie', array('user_login'=>$_POST['user_login']), array('%s') );
