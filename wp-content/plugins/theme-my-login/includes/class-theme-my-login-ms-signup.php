...
class Theme_My_Login_MS_Signup extends Theme_My_Login_Abstract {
	...
	public function tml_display_register( &$template ) {
		...
		echo '<div class="login mu_register aa" id="theme-my-login' . esc_attr( $template->get_option( 'instance' ) ) . '">';
				...
				case 'validate-user-signup' :
					if ( $active_signup == 'all' || $_POST[ 'signup_for' ] == 'blog' && $active_signup == 'blog' || $_POST[ 'signup_for' ] == 'user' && $active_signup == 'user' ) {
						...
						<?php
							...
							if(!empty($_COOKIE['url_redirect']) ) {
								$url_redirect = $_COOKIE['url_redirect'];
							}
						?>
						<p><?php printf( __( 'You will be redirected to <a href="%s">login page</a> in  5 seconds.', 'theme-my-login' ), $url_redirect) ?></p>
						<script>
						...
						<?php//<p> _e( 'But, before you can start using your new username, <strong>you must activate it</strong>.', 'theme-my-login' ) </p>?>
						<?php//<p> printf(__( 'Check your inbox at <strong>%1$s</strong> and click the link given.', 'theme-my-login' ),  $user_email) </p>?>
						<?php//<p> _e( 'If you do not activate your username within two days, you will have to sign up again.', 'theme-my-login' ); </p>?>
						<?php
