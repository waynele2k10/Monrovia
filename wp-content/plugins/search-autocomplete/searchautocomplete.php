	...
	public function acCallback() {
		...
		if ( count( $this->options['autocomplete_posttypes'] ) > 0 ) {
			if ( ( function_exists( 'relevanssi_do_query' ) && ( $this->options['autocomplete_relevanssi'] !== 'false' ) ) ) {
				...
				// Custom Query - use the Relevanssi Log Table to recommend auto-complete
				// 5-26-2015
				$tempPosts = mysql_query("SELECT query, SUM(hits) as totals FROM `wp_relevanssi_log` WHERE query LIKE '$term%' AND `hits` > 0 GROUP BY query ORDER BY totals DESC");
			} else {
			...
			if( mysql_num_rows($tempPosts)>0){
				while($row =  mysql_fetch_array($tempPosts)){
					$url = str_replace(' ', '+', $row['query']);
					$resultsPosts[] = array(
					'title' => $row['query'],
					'url'   => '/?s='.$url,
					);				
				...
				//Get the first word of all past searches and put that on top!
				## To do - Add logic to count number of words in original query and add that to the first one below!!!
				/*$numOfTerms = explode(' ', $term);
				$wordsArray = explode(' ', $resultsPosts[0]['title']);
				$newTerm = '';
				for($i=0;$i<count($numOfTerms);$i++){
					$newTerm .= $wordsArray[$i];
					if($i+1 != count($numOfTerms)){
						$newTerm .=' ';
					}
				}
				$newURL = str_replace(' ', '+', $newTerm);
				$newURL = '/?s='.$newURL;
				$tempArray = array('title' => $newTerm, 'url' => $newURL);
				// First Remove the other instance of it!
				 foreach($resultsPosts as $key => $posts){
					if ( $posts['title'] == $tempArray['title']) {
						unset( $resultsPosts[$key] );
					}
				 }
				//Only add it to the array if its different!
				array_unshift($resultsPosts, $tempArray);
				*/
		}
