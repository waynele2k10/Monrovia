...
class RM_USER_Controller
{
    ...
    {
        //echo "<br>Req<br><pre>", var_dump($request->req), "</pre>";
        //User changed interval or status, reset page number for proper pagination
        ...
        if (isset($request->req['rm_to_search']))
            $search_str = trim($request->req['rm_to_search']);
        else
            $search_str = "";
        $view_data->query_string = $search_str;
        $view_data->interval = isset($request->req['rm_interval']) ? $request->req['rm_interval'] : 'all';
        $view_data->status = isset($request->req['rm_status']) ? $request->req['rm_status'] : 'all';

        /* if (isset($request->user_ids))
          $users = $service->get_all_user_data($req_page, $items_per_page, $search_str, $view_data->status,  $view_data->interval, $request->user_ids);
          else */
        $users = $service->get_all_user_data($req_page, $items_per_page, $search_str, $view_data->status, $view_data->interval);

        $view_data->users = $users;

        /* Pagination values */

        $total_users = count($service->get_all_user_data(1, 99999999, $search_str, $view_data->status, $view_data->interval));
        $view_data->total_users = $total_users;
        $view_data->total_pages = (int) ($total_users / $items_per_page) + (($total_users % $items_per_page) == 0 ? 0 : 1);
