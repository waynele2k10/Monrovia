...
class RM_Forms extends RM_Base_Model
{
    ...
    private $mailchimp_mapped_email;
    private $mailchimp_mapped_first_name;
    private $mailchimp_mapped_last_name;
    ...
    {
        $options_serialized = maybe_unserialize($this->form_options);
    ...
    public function set_form_is_unique_token($tk)
    {
        $this->form_options->form_is_unique_token = false;
    }
    ...
    private function validate_name()
    {
        ...
        {
            $this->errors['NAME'] = 'Form name is empty.';
        }
    ...
    private function validate_description()
    {
        ...
        if (!is_string($this->form_description))
        {
            $this->errors['DESCRIPTION'] = 'Not a string.';
        }

        if (preg_match('/[^a-zA-Z0-9_\-\.]/', $this->form_description))
        {
            $this->errors['DESCRIPTION'] = 'Does not contain valid characters.';
        }
    }
    ...
    private function validate_type()
    {
        ...
        {
            $this->errors['TYPE'] = 'form type is empty.';
        }
        if (!is_int($this->form_type))
        {
            $this->errors['TYPE'] = 'not a valid type.';
        }
    }

    private function validate_custom_text()
    {
        if (!is_string($this->form_custom_text))
        {
            $this->errors['CUSTOM_TEXT'] = 'Not a valid type.';
        }
        if (preg_match('/[^a-zA-Z0-9_\-\.]/', $this->form_custom_text))
        {
            $this->errors['DESCRIPTION'] = 'Does not contain valid characters.';
        }
    }

    private function validate_should_send_email()
    {
        //a check for boolean value should be implemented.
    }
     ...
     */
    public function set_default_form_user_role($rparam)
    {
        if($rparam);
         $this->default_form_user_role = 'subscriber';
    }
