...
// Check if registration fields required
        ...
        foreach ($data->fields_data as $field_data)
        {
            if ($field_data->field_type !== 'Password' || !$data->is_auto_generate)
            {
                ...
                } else if ($field_data->field_type == 'Repeatable')
                {
            ...
            else
            {
                ...
                if ($data->expiry_details->state !== 'perpetual')
                {
                    ...
                    else
                    {
                        ...
                        {
                            case 'both':
                                $exp_str .= '<div class="rm-formcard-expired"><span class="rm_sandclock"></span>' . ($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs) . ' out of ' . $data->expiry_details->sub_limit . ' filled and ' . $data->expiry_details->remaining_days . ' days to go </div>';
                                break;
                            case 'subs':
                                ...
                                $rem = ($rem * 100) / $total;
                                $done = 100 - $rem;
                                $exp_str .= '<div class="rm-formcard-expired"><span class="rm_sandclock"></span>' . ($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs) . ' out of ' . $data->expiry_details->sub_limit . ' filled' . '</div>';
                                break;

                            case 'date':
                                $exp_str .= '<div class="rm-formcard-expired"><span class="rm_sandclock"></span>' . $data->expiry_details->remaining_days . ' days to go' . '</div>';
                                break;
                        }
                    }
