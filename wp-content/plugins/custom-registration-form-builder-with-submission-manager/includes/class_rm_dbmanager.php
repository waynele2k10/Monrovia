...
class RM_DBManager
{
    ...
    public static function get_fields_by_form_id($form_id)
    {
        ...
        $results = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `$foreign_key` = $form_id ORDER BY `field_order`");
    ...
    public static function get_submissions_for_user($user_email, $limit = 9999999, $offset = 0, $sort_by = '', $descending = false)
    {
        ...
        if ($descending === false)
        {
            $results = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `user_email` = '$user_email' ORDER BY `$sort_by` LIMIT $limit OFFSET $offset");
        } else
        {
            $results = $wpdb->get_results("SELECT * FROM `$table_name` WHERE `user_email` = '$user_email' ORDER BY `$sort_by` DESC LIMIT $limit OFFSET $offset");
        }
    ...
    public static function is_expired_by_submissions($form_id, $limit, &$remaining_subs = null)
    {
        global $wpdb;
        $table_name = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
        $num_submissions = $wpdb->get_var("Select count(*) FROM `$table_name` where `form_id` = $form_id");
    ...
    public static function get_results_for_last($interval, $form_id, $field_id, $field_value, $offset = 0, $limit = 999999, $sort_by = 'submission_id', $descending = false)
    {
        ...
        switch (strtolower($interval))
        {
            ...
            case 'all':
                ...
                if ((int) $field_id && $sub_ids)
                    $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id AND `submission_id` in($sub_ids) ";
                elseif ($searched)
                    $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id AND `submission_id` = 0 ";
                else
                    $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id ";
        ...
        if ((int) $field_id && $sub_ids)
            $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id AND `submission_id` in($sub_ids) AND (`submitted_on` $interval_string) ";
        elseif ($searched)
            $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id AND `submission_id` = 0 AND (`submitted_on` $interval_string) ";
        else
            $qry = "SELECT * FROM `$table_name` WHERE `form_id` = $form_id AND (`submitted_on` $interval_string) ";
