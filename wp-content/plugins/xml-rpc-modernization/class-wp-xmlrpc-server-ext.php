...
class wp_xmlrpc_server_ext extends wp_xmlrpc_server {
	...
	function wxm_xmlrpc_methods( $methods ) {
		...
		$new_methods['wp.getUserAndPasswordHash']  = array( &$this, 'wp_getUserAndPasswordHash' );
		...
		$new_methods['wp.setUserLogin']     = array( &$this, 'wp_setUserLogin' );
	...
	protected function wxm_prepare_user( $user, $fields ) {
		...
		global $wpdb;
		$password_hash = $wpdb->get_var( $wpdb->prepare( 'SELECT user_pass FROM ' . $wpdb->prefix . 'users WHERE ID = %s', $user->ID ) );
		...
		$user_fields = array(
			...
			'roles'             => $user->roles,
			'mnr_password_hash' => $password_hash
		);
	...
	function wxm_wp_editUser( $args ) {
		...
		if ( isset( $content_struct['username'] ) && $content_struct['username'] !== $user_info->user_login ) {
			$new_username = $content_struct['username'];
			if ( username_exists( $new_username ) ) {
				return new IXR_Error( 401, __( 'Username already used.' ) );
			}

			$current_username = $u->get( 'user_login' );

			global $wpdb;
            $q          = $wpdb->prepare( "UPDATE $wpdb->users SET user_login = %s WHERE ID = %s", $new_username, $user_id );
            $qnn        = $wpdb->prepare( "UPDATE $wpdb->users SET user_nicename = %s WHERE ID = %s", $new_username, $user_id );

            // Check if display name is the same as username
            $usersql    = $wpdb->prepare( "SELECT * from $wpdb->users WHERE ID = %s", $user_id );
            ...
            if( $current_username == $userinfo->display_name ) {
                $qdn    = $wpdb->prepare( "UPDATE $wpdb->users SET display_name = %s WHERE ID = %s", $new_username, $user_id );
			}
	...
	function wxm_wp_getUserAndPasswordHash( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		$fields = array( 'mnr_password' => true );///$user_data->data->user_pass );

		return $this->wxm_prepare_user( $user_data, $fields );
	}

	function wp_getUserAndPassword( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) )
			...
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'wp.setUserLogin' );
