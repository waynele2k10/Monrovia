...
require_once('../inc/class_plant.php');
	...
	function export_plant_csv($email = "brettex@hotmail.com"){
...
HTML;
		...
		$export_id = date('Ymd');
		$file_handler = fopen("export/plant_database_$export_id.xls", 'w+');
		...
		$result = mysql_query("SELECT id FROM plants");
	...
	function download_export($export_id){
		$file_name = 'plant_database_' . $export_id . '.xls';
