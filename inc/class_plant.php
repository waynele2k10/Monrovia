<?php
	...
	class plant extends record {
		...
		function populate_dumb_values(){
			...
			if(isset($this->info['id'])&&is_numeric($this->info['id'])){
				$this->info['details_url'] = 'http://'.$_SERVER['HTTP_HOST'].'/plant-catalog/plants/'.$this->info['id'].'/'.generate_plant_seo_name($this->info['common_name']);
			} 
			...
			if(is_numeric($this->info['id'])){
				try {
					...
					for($i=0;$i<count($this->special_features);$i++){
						//mysql_query("INSERT INTO plant_special_feature_plants (special_feature_id,plant_id,is_primary) VALUES(".$this->special_features[$i]->id.",".$this->info['id'].")");
					}
					*/
					// UPDATE COMPANION PLANTS
					/*
					mysql_query("DELETE FROM plant_companion_plants WHERE plant_id_1=".$this->info['id']." OR plant_id_2=".$this->info['id']);
					for($i=0;$i<count($this->companion_plant_ids);$i++){
						mysql_query("INSERT INTO plant_companion_plants(plant_id_1,plant_id_2) VALUES(".$this->info['id'].",".$this->companion_plant_ids[$i].")");
					}
					*/
...
//			$this->info['growth_habit'] = get_list_item_value('list_growth_habit',$this->info['growth_habit_id']);
				...
				if($companion_plant->info['status_id']=='1'){
					...
					?>
						<div class="companion_plant_listing">
							<div class="thumbnail" style="background-image:url(<?=$companion_plant->info['image_primary']->info['path_detail_thumbnail']?>)"></div>
							<div class="common_name uppercase"><?=html_sanitize($companion_plant->info['common_name'])?></div>
							<div class="botanical_name"><?=html_sanitize($companion_plant->info['botanical_name'])?></div>
						</div>
						<div class="clear"></div>
					<?
				}
		...
		function output_cms_image_segments_html(){
			...
			if(count($this->image_sets)==0){
				...
				for($i=0;$i<count($this->image_sets);$i++){
					...
					?>
						<div class="plant_image_segment <?=$special_status?>" image_set_id="<?=$image_set->info['id']?>">
							<div style="padding:8px 4px;">
								<div class="thumbnail" style="background-image:url(<?=$image_set->info['path_search_result']?>)"></div>
								<div class="details">
									<div class="details_action"><a href="javascript:edit_plant_image(<?=$image_set->info['id']?>);void(0);">[edit]</a> <a href="javascript:edit_plant_image_delete(<?=$image_set->info['id']?>);void(0);">[delete]</a></div>
									<div class="details_title"><?=$image_set->info['title']?></div>
									...
									<div class="details_credit">Credit: <?=$image_set->info['photography_credit']?></div>
									<div class="details_expiration_date">Expiration Date: <?=$image_set->info['expiration_date']?></div>
									<div class="details_source">Source: <?=$image_set->info['source']?></div>

									<div class="details_links">
										Image sizes:<br />
										<ul>
											<li><a href="<?=$image_set->info['path_original']?>" target="_blank">Original</a></li>
											<li><a href="javascript:void(0);" onclick="lightview_show_image('<?=$image_set->info['path_detail']?>');">Plant details</a></li>
											<li><a href="javascript:void(0);" onclick="lightview_show_image('<?=$image_set->info['path_search_result']?>');">Search results</a></li>
											<li><a href="javascript:void(0);" onclick="lightview_show_image('<?=$image_set->info['path_detail_thumbnail']?>');">Plant details thumbnail</a></li>
										</ul>
									</div>
								</div>
							</div>
				...
				for($i=0;$i<count($this->image_sets);$i++){
					...
					if($image_set->info['is_active']=='1'){
						...
						switch($version){
							case 'desktop':						
								?>
									<a title="<?=$caption?>" href="<?=$image_set->info['path_detail']?>" onclick="lightview_show_image('<?=str_replace('\'','\\\'',$image_set->info['path_detail'])?>','<?=str_replace('\'','\\\'',$title)?>','<?=str_replace('\'','\\\'',$credit)?>');return false;"><img class="imgThumb" src="/img/spacer.gif" hires="<?=$image_set->info['path_detail']?>" alt="" style="background-image:url(<?=$image_set->info['path_search_result']?>);" /></a><!-- PRELOAD --><!--<img src="<?=$image_set->info['path_detail']?>" style="position:absolute;visibility:hidden;" />--><!-- /PRELOAD -->
								<?
							break;
							case 'mobile':
								?>
									<div class="item">
										<a href="javascript:monrovia.ui.launch_plant_photo('<?=$image_set->info['path_detail']?>','<?=$title?>','<?=$credit?>');void(0);"><img src="<?=$image_set->info['path_detail_thumbnail']?>" alt="<?=$title?>" /></a>
									</div>
								<?
							break;
						}
		...
		function update_keywords(){
			...
			mysql_query("UPDATE plants SET keywords='$keywords' WHERE id=".$this->info['id']);
		}
