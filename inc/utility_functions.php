...
//		$value = str_replace_unicode(8482,chr(153), $value); // CAUSES STRINGS TO CUT OFF AT TMs
	...
	function generate_email_html($message){
		return str_replace('{message}',$message,file_get_contents($GLOBALS['server_info']['physical_root'] . 'community/email_templates/generic.htm'));
	}
	...
	?>
		<style>#<?=$link_id?> a { color:#a1cc50!important; cursor:default; }</style>
	<?php
	}
		...
		try {
			...
			if ($google_maps_result->status != 'OK' ){
				if($google_maps_result->status=='OVER_QUERY_LIMIT'){
					...
					$api_result = json_decode(file_get_contents('http://www.mapquestapi.com/geocoding/v1/address?key='.$GLOBALS['mapquest_api_key'].'&location='.urlencode($query)));
