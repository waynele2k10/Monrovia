<?php
	...
	function sql_connect(){
		@mysql_connect($GLOBALS['db_host'], $GLOBALS['db_user'], $GLOBALS['db_pass']) or die ('<h3>Unable to connect to database.</h3>');
		@mysql_select_db($GLOBALS['db_name']) or die ('<h3>Unable to connect to database.</h3>');
	}
	...
	function get_list_item_value($table_name,$id,$col_name = 'name'){
		...
		if(!is_suspicious(array($table_name,$id,$col_name))){
			...
			$ret = @mysql_result(sql_query($query),0,$col_name);
	...
	function get_list_item_id($table_name,$value,$col_name = 'name'){
		...
		if(!is_suspicious(array($table_name,$value,$col_name))){
			...
			if(isset($GLOBALS['sql_cache'][$query])) return $GLOBALS['sql_cache'][$query];
			$ret = @mysql_result(sql_query($query),0,'id');
			$GLOBALS['sql_cache'][$query] = $ret;
...
//		$value = str_replace_unicode(8482,chr(153), $value); // CAUSES STRINGS TO CUT OFF AT TMs
		...
		// DASH
		$value = str_replace(chr(149),"-",$value);
		$value = str_replace(chr(150),"-",$value);
		$value = str_replace(chr(151),"--",$value);

		$value = str_replace(chr(96), chr(39), $value);
		$value = str_replace(chr(145), chr(39), $value);
		$value = str_replace(chr(146), chr(39), $value);
		$value = str_replace(chr(180), chr(39), $value);

		// OTHER
		$value = str_replace(chr(160),' ', $value);
		$value = str_replace(chr(133),'...', $value);
		$value = str_replace(chr(130),'\'', $value);
		...
		//$value = str_replace(chr(153),'hey', $value);
	...
	function output_select_options($table_name){
		...
		for($i=0;$i<count($list_records);$i++){
			echo("<option value=\"".$list_records[$i]['id']."\" attribute_name=\"".html_sanitize($list_records[$i]['name'])."\" attribute_is_historical=\"".$list_records[$i]['is_historical']."\">".html_sanitize($list_records[$i]['name'])."</option>");
	...
	function multiselect_to_attributes($field_data){
		...
		for($i=0;$i<count($field_data);$i++){
			$ret[] = new plant_attribute($field_data[$i],null,null,null);
		}
		return $ret;
	}
...
///////////////////////////////
	...
	function get_columns_names($table_name) {
		$result = sql_query("SELECT GROUP_CONCAT(COLUMN_NAME) AS column_names FROM information_schema.columns WHERE TABLE_NAME='$table_name'");
		return @explode(',',mysql_result($result,0,'column_names'));
    }

	function get_table_column_info($table_name){
		$ret = array();
		$result = sql_query("SELECT COLUMN_NAME AS col_name, DATA_TYPE AS col_type, CHARACTER_MAXIMUM_LENGTH AS col_maxlength FROM information_schema.columns WHERE TABLE_NAME='$table_name'");
	...
	function get_list_table_names() {
		$result = sql_query("SELECT GROUP_CONCAT(DISTINCT TABLE_NAME) AS table_names FROM information_schema.columns WHERE TABLE_NAME like 'list_%'");
