			...
			//if(!is_suspicious(array($this->table_name,$this->table_fields))){
				if(isset($this->info['id'])&&$this->info['id']!=''){
					if(is_numeric($this->info['id'])){
						$ret = sql_query("UPDATE $this->table_name SET $values WHERE id=".$this->info['id']);
					}
				}else{
					//echo $this->table_fields;
					//echo $this->table_name;
					//echo $values;
					$ret = sql_query("INSERT INTO $this->table_name ($this->table_fields) VALUES ($values)");
