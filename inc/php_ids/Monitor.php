...
class IDS_Monitor
{
    ...
    public function __construct(array $request, IDS_Init $init, array $tags = null)
    {
        ...
        include_once $GLOBALS['server_info']['physical_root'].'inc/php_ids/Report.php';
        ...
        if (!is_array($value)) {
            if (is_string($value)) {
                ...
                if ($filter = $this->_detect($key, $value)) {
                    include_once $GLOBALS['server_info']['physical_root'].'inc/php_ids/Event.php';
    ...
    private function _detect($key, $value)
    {
        ...
        // use the converter
        include_once $GLOBALS['server_info']['physical_root'].'inc/php_ids/Converter.php';
    ...
    private function _jsonDecodeValues($key, $value) {
        ...
        $tmp_key   = from_json($key);
        $tmp_value = from_json($value);
